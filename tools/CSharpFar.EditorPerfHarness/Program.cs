...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        foreach (bool syntax in new[] { false, true })
        {
            ...
            runs.AddRange(Repeat("cursor-only" + suffix, normal, 120, 40, syntax, cursorOnly: true));
            runs.AddRange(Repeat("scrolling" + suffix, normal, 120, 40, syntax, cursorOnly: false));
        }
        ...
        foreach (int lines in new[] { 100, 1000, 5000, 20000 })
            runs.AddRange(Repeat($"scrolling-lines-{lines}", fixtures.PathFor(lines, FixtureLength.Normal), 120, 40, false, false));
        foreach ((int width, int height) in new[] { (80, 25), (120, 40), (180, 50) })
            runs.AddRange(Repeat($"scrolling-viewport-{width}x{height}", normal, width, height, false, false));
        foreach (FixtureLength length in new[] { FixtureLength.Short, FixtureLength.Normal, FixtureLength.Long })
            runs.AddRange(Repeat($"scrolling-{length.ToString().ToLowerInvariant()}-lines", fixtures.PathFor(5000, length), 120, 40, false, false));
    ...
    private static IEnumerable<RunResult> Repeat(string name, string filePath, int width, int height, bool syntax, bool cursorOnly)
    {
        for (int repetition = 0; repetition < Repetitions; repetition++)
        {
            System.Console.Error.WriteLine($"{name} ({repetition + 1}/{Repetitions})");
            yield return Execute(name, filePath, width, height, syntax, cursorOnly, repetition);
        }
    }

    private static RunResult Execute(string name, string filePath, int width, int height, bool syntax, bool cursorOnly, int repetition)
    {
        ...
        return new RunResult(name, repetition, filePath, width, height, syntax, cursorOnly, stopwatch.Elapsed, allocated, frames, metrics);
    }
...
internal sealed record RunResult(string Name, int Repetition, string FilePath, int Width, int Height, bool Syntax, bool CursorOnly, TimeSpan Elapsed, long Allocated, IReadOnlyList<EditorPerformanceFrameMeasurement> Frames, EditorPerformanceMetrics Metrics);
...
internal static class Report
{
    public static string Create(IReadOnlyList<RunResult> runs, FixtureGenerator fixtures, string artifactDirectory)
    {
        ...
        sb.AppendLine("- Configuration: Release; deterministic in-memory `FakeConsoleDriver`; full editor/session/composition pipeline through the public production `ScreenRenderer` configuration.");
        ...
        sb.AppendLine();
        sb.AppendLine("| Scenario | ms/frame p50 | p95 | p99 | Alloc/frame | Full/partial |");
        sb.AppendLine("| --- | ---: | ---: | ---: | ---: | ---: |");
        foreach (var group in runs.GroupBy(run => run.Name))
        {
            var data = Aggregate(group.ToArray());
            sb.AppendLine($"| {group.Key} | {data.P50:F3} | {data.P95:F3} | {data.P99:F3} | {data.Alloc:F0} B | {data.Full:F0}/{data.Partial:F0} |");
        }
        sb.AppendLine();
        ...
        sb.AppendLine("Visible editor lines use the production sequential Unicode renderer; the benchmark keeps the complete editor, composition, and public renderer/presentation path intact.");
        sb.AppendLine();
    ...
    private static AggregateResult Aggregate(IReadOnlyList<RunResult> runs)
    {
        ...
        return new AggregateResult(
            ...
            runs.Average(run => run.Allocated / (double)Math.Max(1, run.Frames.Count)));
    }
    ...
    private readonly record struct AggregateResult(double P50, double P95, double P99, double Full, double Partial, double Render, double Viewport, double SyntaxTime, double Draw, double Lines, double Input, double Classification, double Alloc);
}
