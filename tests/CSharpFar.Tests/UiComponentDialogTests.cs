...
public sealed class UiComponentDialogTests
{
    ...
    public void ScrollableFormDialog_KeepsFocusedRowVisible()
    {
        var form = new ScrollableFormDialog(
            bodyRowCount: 20,
            focusRowCount: 5,
            focusRowToVirtualRow: row => row * 4);

        form.SetFocusRow(4, viewportRows: 5);

        Assert.Equal(4, form.FocusRow);
        Assert.Equal(12, form.BodyScrollTop);
    }

    [Fact]
    public void ScrollableFormDialog_SkipsNonFocusableRows()
    {
        var form = new ScrollableFormDialog(
            bodyRowCount: 10,
            focusRowCount: 4,
            focusRowToVirtualRow: row => row,
            isFocusableRow: row => row != 2);

        form.SetFocusRow(1, viewportRows: 3);
        form.MoveFocus(delta: 1, viewportRows: 3);

        Assert.Equal(3, form.FocusRow);
    }

    [Fact]
    public void ScrollableFormDialog_MouseWheelScrollsBodyWithoutChangingFocus()
    {
        var form = new ScrollableFormDialog(
            bodyRowCount: 20,
            focusRowCount: 3,
            focusRowToVirtualRow: row => row);
        form.SetFocusRow(0, viewportRows: 5);

        bool handled = form.TryHandleWheel(
            new MouseConsoleInputEvent(0, 0, MouseButton.WheelDown, MouseEventKind.Wheel, MouseKeyModifiers.None),
            viewportRows: 5);

        Assert.True(handled);
        Assert.Equal(0, form.FocusRow);
        Assert.Equal(3, form.BodyScrollTop);
    }

    [Fact]
    public void ScrollableFormDialog_ScrollbarClickUpdatesBodyScroll()
    {
        var form = new ScrollableFormDialog(
            bodyRowCount: 20,
            focusRowCount: 3,
            focusRowToVirtualRow: row => row);
        var bounds = new Rect(10, 2, 1, 6);

        bool handled = form.TryHandleScrollbarMouse(
            new MouseConsoleInputEvent(10, 6, MouseButton.Left, MouseEventKind.Down, MouseKeyModifiers.None),
            bounds,
            viewportRows: 6);

        Assert.True(handled);
        Assert.True(form.BodyScrollTop > 0);
    }
