...
public class FileViewerTests : IDisposable
{
    ...
    [Fact]
    public void Show_WatchRefreshesSameLengthExternalOverwrite()
    {
        string path = Write(
            "watch-overwrite.txt",
            "AAAA\nline2\nline3\nline4\nline5\nline6\nline7\nline8\n",
        ...
        driver.BeforeReadInput = current =>
        {
            ...
            {
                current.EnqueueKey(Key(ConsoleKey.F10));
            }
        };
        driver.EnqueueKey(Key(ConsoleKey.F, 'f'));
        ...
        Assert.Contains("BBBB", WrittenText(driver));
    }

    [Fact]
    public void Show_TailRefreshesAppendAndStaysAtEnd()
    {
        string path = Write(
            "tail-append.txt",
        ...
        driver.BeforeReadInput = current =>
        {
            ...
            if (readCount == 2)
            {
                File.AppendAllText(path, "tail-appended\n", new UTF8Encoding(false));
            }
            else if (readCount == 3)
            {
                current.EnqueueKey(Key(ConsoleKey.F10));
        ...
        string content = driver.GetRegionText(new CSharpFar.Console.Models.Rect(0, 1, 80, 6));
        Assert.Contains("tail-appended", content);
        Assert.Contains("TAIL", WrittenText(driver));
    }

    [Fact]
    public void Show_WatchRetriesDeleteThenRecreate()
    {
        string path = Write(
            "watch-recreate.txt",
            "old-value\nline2\nline3\nline4\nline5\nline6\nline7\nline8\n",
        ...
        Assert.Contains("new-value", WrittenText(driver));
    }
