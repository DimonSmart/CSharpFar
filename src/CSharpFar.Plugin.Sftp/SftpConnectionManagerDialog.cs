...
internal sealed class SftpConnectionManagerDialog
{
        ...
        int focusedButton)
    {
        ...
        _modalRenderer.Render(_screen, geometry.Bounds, "SFTP connections", true, FarDialogStyles.OuterOptions, FarDialogStyles.FrameOptions, (_, layout) =>
        {
            ...
            if (connections.Count == 0)
            {
                _screen.Write(
                    ...
                    "No saved SFTP connections.".PadRight(geometry.ListBounds.Width),
                    fill);
            }
            else
            {
    ...
    private static bool TryCreateButtonResult(
        ...
        IReadOnlyList<SftpConnectionInfo> connections,
        int cursor,
        out SftpConnectionManagerResult? result)
    {
        result = null;
        switch (buttonId)
        {
            case "cancel":
                return true;
            case "create":
                result = new SftpConnectionManagerResult(SftpConnectionManagerAction.Create, null);
                return true;
            case "connect":
                if (TrySelected(connections, cursor, out var connectConnection))
                {
                    result = new SftpConnectionManagerResult(SftpConnectionManagerAction.Connect, connectConnection);
                    return true;
                }
                return false;
            case "edit":
                if (TrySelected(connections, cursor, out var editConnection))
                {
                    result = new SftpConnectionManagerResult(SftpConnectionManagerAction.Edit, editConnection);
                    return true;
                }
                return false;
            case "delete":
                if (TrySelected(connections, cursor, out var deleteConnection))
                {
                    result = new SftpConnectionManagerResult(SftpConnectionManagerAction.Delete, deleteConnection);
                    return true;
                }
                return false;
            default:
